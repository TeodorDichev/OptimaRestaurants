using System.ComponentModel.DataAnnotations;

namespace webapi.DTOs.Account
{
    public class RegisterEmployeeDto
    {
        [StringLength(30, MinimumLength = 2, ErrorMessage = "First name must be at least {2}, and maximum {1} characters")]
        public required string FirstName { get; set; }
        [StringLength(30, MinimumLength = 2, ErrorMessage = "Last name must be at least {2}, and maximum {1} characters")]
        public required string LastName { get; set; }
        [StringLength(30, MinimumLength = 2, ErrorMessage = "City must be at least {2}, and maximum {1} characters")]
        public required string City { get; set; }
        public required DateTime BirthDate { get; set; }
        [RegularExpression("^\\w+@[a-zA-Z_]+?\\.[a-zA-Z]{2,3}$", ErrorMessage = "Invalid email address")]
        public required string Email { get; set; }
        [StringLength(30, MinimumLength = 6, ErrorMessage ="Password must be at least {2}, and maximum {1} characters")]
        public required string Password { get; set; }
    }
}
